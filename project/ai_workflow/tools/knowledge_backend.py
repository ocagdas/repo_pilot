...
def context(repo, user_config=None, overrides=None):
    ...
    return {'repo': str(repo), 'repository_id': config['repository_id'],
            'revision': git(repo, 'rev-parse', 'HEAD'),
            'dirty': bool(git(repo, 'status', '--porcelain', '--untracked-files=normal')),
            'backend': backend, 'data_dir': str(data), 'settings': values}


def cgc_env(ctx):
    return {**os.environ, 'KUZUDB_PATH': str(Path(ctx['data_dir']) / 'kuzudb'),
            'DEFAULT_DATABASE': 'kuzudb', 'CGC_ALLOWED_ROOTS': ctx['repo'],
            'CGC_RUNTIME_DB_TYPE': 'kuzudb',
            'CGC_RUNTIME_DB_PATH': str(Path(ctx['data_dir']) / 'kuzudb')}


def cgc_command(ctx, *args):
    return [ctx['settings']['cgc']['executable'], '--database', 'kuzudb',
            '--db-path', str(Path(ctx['data_dir']) / 'kuzudb'), *args]
...
def status(ctx):
    ...
    current = bool(record and record.get('revision') == ctx['revision']
                   and record.get('repo') == ctx['repo']
                   and record.get('repository_id') == ctx['repository_id']
                   and (Path(ctx['data_dir']) / 'kuzudb').exists() and not ctx['dirty'])
    return {k: ctx[k] for k in ('backend', 'repo', 'repository_id', 'revision', 'dirty')} | {
        'storage': ctx['data_dir'] if ctx['backend'] == 'cgc' else (
            ctx['settings']['sourcegraph']['url'] if ctx['backend'] == 'sourcegraph' else None),
        ...
        'source_fallback_required': ctx['backend'] in ('off', 'sourcegraph') or ctx['dirty'] or (
            ctx['backend'] == 'cgc' and not current)}


def check_cgc_version(ctx):
    proc = subprocess.run([ctx['settings']['cgc']['executable'], '--version'], capture_output=True,
...
def index(ctx):
    ...
    data = Path(ctx['data_dir']); data.mkdir(parents=True, exist_ok=True)
    marker = data / 'repo-pilot.json'
    # Invalidate first: failed or interrupted rebuilding must not leave a current marker.
    marker.unlink(missing_ok=True)
    subprocess.run(cgc_command(ctx, 'index', ctx['repo'], '--force'), cwd=ctx['repo'],
                   env=cgc_env(ctx), check=True, stdout=sys.stderr, timeout=3600)
    if git(ctx['repo'], 'rev-parse', 'HEAD') != ctx['revision'] or git(
            ctx['repo'], 'status', '--porcelain', '--untracked-files=normal'):
        raise SettingsError('Checkout changed during indexing (CGC may have created .cgcignore); review and commit exclusions, then retry.')
    marker.write_text(json.dumps({k: ctx[k] for k in ('repo', 'repository_id', 'revision')},
                                 indent=2) + '\n', encoding='utf-8')
    return status(ctx)
...
def bundle(ctx, operation, filename):
    ...
    if operation == 'export':
        ...
        try:
            with redirect_stdout(sys.stderr):
                ok, _ = CGCBundle(manager).export_to_bundle(path, repo_path=Path(state(ctx).get('graph_root', ctx['repo'])))
            if not ok:
        ...
        record = {'schema_version': '1.0', 'backend': 'cgc', 'cgc_version': '0.6.13',
                  'repository_id': ctx['repository_id'], 'revision': ctx['revision'],
                  'sha256': digest}
        with sidecar.open('x', encoding='utf-8') as handle:
            handle.write(json.dumps(record, indent=2) + '\n')
        return {'bundle': str(path), 'manifest': str(sidecar), **record}
    record = json.loads(sidecar.read_text(encoding='utf-8'))
    for key, expected in [('schema_version', '1.0'), ('backend', 'cgc'), ('cgc_version', '0.6.13'),
                          ('repository_id', ctx['repository_id']), ('revision', ctx['revision'])]:
    ...
    with tempfile.TemporaryDirectory(prefix='.cgc-import-', dir=data.parent) as temporary:
        ...
        (staging / 'repo-pilot.json').write_text(json.dumps(
            {k: ctx[k] for k in ('repo', 'repository_id', 'revision')} | {'graph_root': graph_root}) + '\n', encoding='utf-8')
        staging.rename(data)
    return status(ctx)
...
def query_request(ctx, query, limit):
    ...
    return 'keyword_search', {'query': f'repo:{repo_filter} rev:{ctx["revision"]} count:{limit} patternType:literal {json.dumps(query)}'}
    ...
    async with AsyncExitStack() as stack:
        if ctx['backend'] == 'cgc':
            streams = await stack.enter_async_context(stdio_client(StdioServerParameters(
                ...
                args=cgc_command(ctx, 'mcp', 'start')[1:], env=cgc_env(ctx), cwd=ctx['repo'])))
        else:
            sg = ctx['settings']['sourcegraph']
            ...
            client = await stack.enter_async_context(httpx.AsyncClient(
                headers={'Authorization': 'token ' + token}, follow_redirects=False, timeout=60))
            streams = await stack.enter_async_context(streamable_http_client(
                sg['url'].rstrip('/') + '/.api/mcp', http_client=client))
...
def database_lock(ctx):
    ...
    try:
        ...
        raise SettingsError('CGC database busy; after a crash remove its .lock only once all users have stopped.') from exc
    try:
        with handle:
...
def main(argv=None):
    ...
    try:
        ...
        if args.command == 'status':
            ...
            if ctx['dirty']:
                raise SettingsError('Checkout has local changes; read source files until a matching clean revision is available.')
            with database_lock(ctx):
            ...
            result = status(ctx) | {'content': response if len(rendered) <= args.max_chars else rendered[:args.max_chars],
        ...
        # Transport exceptions may contain authorization headers; never echo those.
        print(json.dumps({'error': 'Backend transport failed; check installation, endpoint and access.',
                          'source_fallback_required': True}), file=sys.stderr)
        return 2
