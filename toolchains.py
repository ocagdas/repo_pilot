...
def run(argv, cwd=None):
    ...
    if result.returncode:
        raise ToolchainError(f'Command failed ({argv[0]}): {result.stdout}{result.stderr}')
...
def resolve_selection(ref=None, record_file=None):
    ...
    if record_file:
        ...
        if ref and ref not in (data.get('requested_ref'), data['upstream_commit'],
                               data['specify_cli_version'], 'v' + data['specify_cli_version']):
        ...
        return {key: data[key] for key in ('schema_version', 'requested_ref', 'upstream_repository',
                                         'upstream_commit', 'specify_cli_version', 'package_version')}
    if ref is None or ref in (pin['upstream_tag'], pin['upstream_commit'], pin['specify_cli_version']):
        ...
        if not SHA.fullmatch(ref):
            ...
            refs = {}
            for line in run(['git', 'ls-remote', '--tags', OFFICIAL + '.git', f'refs/tags/{tag}', f'refs/tags/{tag}^{{}}']).splitlines():
                sha, name = line.split()
                refs[name] = sha
    ...
    return validate_record({'schema_version': '1.0', 'requested_ref': ref,
                           'upstream_repository': OFFICIAL, 'upstream_commit': commit,
                           'specify_cli_version': version, 'package_version': pin['package_version']})
...
def discover_cli(settings, selection, explicit=None):
    ...
    if configured['mode'] == 'conda':
        ...
        if conda:
            try:
                prefix = run([conda, 'run', '-n', conda_environment(settings, selection),
    ...
    raise ToolchainError('No matching Specify CLI found; run setup_tooling.py for the selected '
...
def probe_cli(specify, selection):
    output = re.sub(r'\x1b\[[0-9;]*m', '', run([specify, 'version'], ROOT))
    ...
    for python in candidates:
        if not python.is_file():
            continue
        metadata = json.loads(run([python, '-c',
        ...
        if (metadata['version'] == expected and source.get('url', '').removesuffix('.git') == OFFICIAL
    ...
    if is_override(selection) and source_status != 'verified_commit':
        raise ToolchainError('Override CLI source commit could not be verified; run setup_tooling.py for the selected ref')
...
def stage_package(specify, integrations, selection, workspace):
    ...
    for name, manifest in [('extension', 'extension.yml'), ('preset', 'preset.yml')]:
        ...
        if is_override(selection):
            ...
            amended, count = re.subn(r'(?m)^(\s*speckit_version:)\s*==[^\s]+\s*$',
                                     lambda m: m[1] + ' ==' + selection['specify_cli_version'], original)
            if count != 1:
                raise ToolchainError('Cannot adapt staging compatibility declaration')
            manifest_path.write_text(amended, encoding='utf-8')
        packages[name] = path
    run([specify, 'init', stage, '--integration', integrations[0], '--script', 'py',
         '--ignore-agent-tools', '--non-interactive'], ROOT)
    ...
    evidence = {'status': 'pass', 'classification': 'local_override_checked' if is_override(selection) else 'distribution_default',
                'integrations': integrations, 'source_verification': source_status,
                'package_fingerprint': package_fingerprint(),
                'checks': ['CLI version', 'init', 'extension', 'preset', 'integration activation', 'command composition', 'feature templates'],
