...
8. A repository layout and CI workflow suitable for pushing to your own Git host. GitHub Actions configuration is included for GitHub.
...
The current inventory overlays are real file records. They are not a compiler dependency graph. CodeGraphContext and other backends were researched but have not been integrated.

## Validation scope

Linux execution with the pinned official CLI is tested. Windows and macOS instructions, native dependency installation, Conda creation and Docker builds are supplied but not verified here. CI is ready to run after pushing; no remote CI success is claimed. See VALIDATION.md for exact evidence and test commands.
