...
class CompletionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.report = {'schema_version':'1.0','source_revision':'abc','snapshot':'abc:clean',
          'tasks':[{'id':'T001','status':'completed','evidence':['test.log'],'remaining_work':''},
                   {'id':'T002','status':'untouched','evidence':[],'remaining_work':'Independent review'}],
          'acceptance_criteria':[{'id':'AC001','status':'completed','evidence':['test.log'],'remaining_work':''}],
          'checks':[], 'review':{'kind':'none','result':'pending','evidence':''}, 'changed_files':[], 'limitations':['Review pending']}
    def tearDown(self): self.temp.cleanup()
    def run_report(self):
        (self.feature / 'completion.json').write_text(json.dumps(self.report))
        module.validate(self.feature)
    def test_valid_partial_report(self): self.run_report()
