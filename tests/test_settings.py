...
class SettingsTests(unittest.TestCase):
    ...
    def test_precedence_at_every_boundary(self):
        ...
        self.write(self.user, {'tooling': {'conda_name': 'user'}}, projects={'team:repo': {'tooling': {'conda_name': 'personal'}}})
    ...
    def test_remote_identity_removes_credentials_and_protocol(self):
        self.assertEqual(canonical_remote('git@example.com:Team/repo.git'), canonical_remote('https://user:secret@example.com/Team/repo.git'))
        self.assertIsNone(canonical_remote('/tmp/project'))

    def test_malformed_and_unsupported_policy_rejected(self):
        for value in ({'quality_gates': {'required': []}}, {'knowledge': {'mode': True}}, {'tooling': {'conda_name': 'a;echo'}}):
    ...
    def test_inspect_has_no_writes_and_fingerprint_ignores_provenance(self):
        ...
        result = subprocess.run([sys.executable, str(ROOT / 'configure.py'), 'inspect', '--repo', str(self.repo), '--user-config', str(self.user)], capture_output=True, text=True, encoding='utf-8', check=True)
    ...
    def test_setup_uses_shared_resolver_and_explicit_cli_wins(self):
        ...
        result = subprocess.run([sys.executable, str(ROOT / 'setup_tooling.py'), '--user-config', str(self.user), '--conda-name', 'explicit'], capture_output=True, text=True, encoding='utf-8', check=True)
