...
from settings import SettingsError, resolve, validate_settings
...
class KnowledgeTests(unittest.TestCase):
    ...
    def mark_current(self, ctx):
        data = Path(ctx['data_dir']); data.mkdir(parents=True)
        (data / 'kuzudb').touch()
        (data / 'repo-pilot.json').write_text(json.dumps(
            {key: ctx[key] for key in ('repo', 'revision', 'repository_id')}), encoding='utf-8')

    def test_off_never_launches_backend_or_writes_cache(self):
        with patch.object(kb, 'query_backend', side_effect=AssertionError('launched')), patch.object(kb, 'index', side_effect=AssertionError('indexed')):
    ...
    def test_config_selection_cli_override_and_source_policy(self):
        self.user.write_text(json.dumps({'schema_version': '1.0', 'settings': {
            'knowledge': {'backend': 'cgc'}}}), encoding='utf-8')
    ...
    def test_invalid_backend_and_unsafe_credentials_rejected(self):
        for values in ({'knowledge': {'backend': []}}, {'knowledge': {'backend': 'unknown'}},
    ...
    def test_missing_dirty_and_other_clone_snapshots_rejected(self):
        ctx = self.ctx()
        with self.assertRaises(SettingsError): kb.query_request(ctx, 'hello', 20)
        ...
        with self.assertRaises(SettingsError): kb.query_request(self.ctx(), 'hello', 20)
    ...
    def test_lock_prevents_concurrent_mutation(self):
        ...
        with kb.database_lock(ctx):
            with self.assertRaises(SettingsError):
                with kb.database_lock(ctx): pass
        self.assertFalse(Path(ctx['data_dir'] + '.lock').exists())

    def test_version_probe_supports_stderr_and_rejects_other_versions(self):
        ctx = self.ctx()
        with patch.object(kb.subprocess, 'run', return_value=subprocess.CompletedProcess(
                [], 0, stdout='', stderr='CodeGraphContext 0.6.13\n')):
            kb.check_cgc_version(ctx)
        with patch.object(kb.subprocess, 'run', return_value=subprocess.CompletedProcess(
                [], 0, stdout='CodeGraphContext 0.5.0', stderr='')):
            with self.assertRaises(SettingsError): kb.check_cgc_version(ctx)

    def test_failed_index_invalidates_marker(self):
        ctx = self.ctx(); self.mark_current(ctx)
        with patch.object(kb, 'check_cgc_version'), patch.object(kb.subprocess, 'run', side_effect=subprocess.CalledProcessError(1, 'cgc')):
            with self.assertRaises(subprocess.CalledProcessError): kb.index(ctx)
    ...
    def test_imported_paths_map_to_clone_and_reject_missing_source(self):
        ...
        marker.write_text(json.dumps(json.loads(marker.read_text(encoding='utf-8')) |
                                     {'graph_root': '/virtual/bundle/producer-name'}), encoding='utf-8')
    ...
    def test_bundle_rejects_wrong_identity_revision_checksum_and_existing_data(self):
        ...
        metadata = {'schema_version': '1.0', 'backend': 'cgc', 'cgc_version': '0.6.13',
                    'repository_id': ctx['repository_id'], 'revision': ctx['revision'],
                    'sha256': hashlib.sha256(b'fixture').hexdigest()}
        manager = MagicMock()
        modules = {'codegraphcontext.core.database_kuzu': SimpleNamespace(KuzuDBManager=manager),
        ...
        with patch.dict(sys.modules, modules), patch.object(kb, 'version', return_value='0.6.13'):
            for field in ('repository_id', 'revision', 'sha256', 'cgc_version'):
                ...
                with self.assertRaises(SettingsError): kb.bundle(ctx, 'import', str(artifact))
    ...
    def test_import_cleans_staging_on_metadata_constructor_and_close_failures(self):
        ...
        for failure in ('metadata', 'constructor', 'close'):
            with self.subTest(failure=failure):
                ...
                manager = MagicMock()
                if failure == 'constructor': manager.side_effect = RuntimeError('constructor failed')
                if failure == 'close': manager.return_value.close_driver.side_effect = RuntimeError('close failed')
                adapter = MagicMock()
                adapter.return_value.import_from_bundle.return_value = (True, 'ok')
                modules = {'codegraphcontext.core.database_kuzu': SimpleNamespace(KuzuDBManager=manager),
                    'codegraphcontext.core.cgc_bundle': SimpleNamespace(CGCBundle=adapter,
                        _default_bundle_install_root=lambda *args: 'virtual/root')}
                with patch.dict(sys.modules, modules), patch.object(kb, 'version', return_value='0.6.13'):
    ...
    def test_transport_failure_does_not_echo_credentials(self):
        async def fail(*args): raise RuntimeError('token super-secret')
    ...
    def test_output_budget_and_checkout_race(self):
        async def response(*args): return {'text': 'a' * 1000}
...
class SourcegraphTransportTests(unittest.TestCase):
    ...
    def test_mcp_handshake_and_authenticated_tool_call(self):
        ...
        with patch.dict(os.environ, {'SOURCEGRAPH_TOKEN': 'fixture-token'}), patch.object(httpx, 'AsyncClient', factory):
