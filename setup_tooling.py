...
def main():
    ...
    parser.add_argument('--conda-name', default=None,
                        help='Conda environment name (resolved default: spec_kit_engineering)')
    ...
    modes.add_argument('--install-mode', choices=['static', 'editable'], default='static',
    ...
    parser.add_argument('--extras', choices=['minimal', 'cgc', 'sourcegraph', 'all'], default='minimal',
    ...
    try:
        ...
        explicit = {key: str(value) if isinstance(value, Path) else value
                    for key, value in {'mode': args.mode, 'env_dir': args.env_dir,
                                       'conda_name': args.conda_name}.items() if value is not None}
        ...
        if install_mode == 'conda' and toolchains.is_override(selection):
            conda = os.environ.get('CONDA_EXE') or shutil.which('conda') or 'conda'
            plan = [[conda, 'create', '--name', args.conda_name, '--yes', '--override-channels', '-c', 'conda-forge', 'python=3.12', 'pip', 'git'],
                    [conda, 'run', '-n', args.conda_name, 'python', '-m', 'pip', 'install', toolchains.requirement(selection)]]
        else:
            plan = commands(install_mode, args.env_dir, args.conda_name)
            if toolchains.is_override(selection):
                plan[-1] = [str(toolchains.python_in(args.env_dir)), '-m', 'pip', 'install', toolchains.requirement(selection)]
        plan.append(package_command(install_mode, args.env_dir, args.conda_name,
                                    args.install_mode, args.extras))
        ...
        if install_mode == 'venv' and args.env_dir.exists():
            if record_path.is_file():
                ...
                raise ValueError('Override environment already exists without an ownership record; choose a different --env-dir')
        print(json.dumps({'mode':args.mode, 'effective_install_mode':install_mode, 'commands':plan,
                          'toolchain':selection, 'environment': str(args.env_dir) if install_mode == 'venv' else args.conda_name,
                          'install_mode': args.install_mode, 'extras': args.extras,
                          'apply':args.apply}, indent=2), flush=True)
        if not args.apply:
            if args.export_record:
                raise ValueError('--export-record requires --apply; preview output already includes the resolved source')
            return
        ...
        try:
            ...
            if install_mode == 'venv':
                ...
                conda = os.environ.get('CONDA_EXE') or shutil.which('conda') or 'conda'
                prefix = Path(toolchains.run([conda, 'run', '-n', args.conda_name, 'python', '-c', 'import sys; print(sys.prefix)']))
        ...
        if install_mode == 'venv':
            print('Launcher: ' + str(args.env_dir / ('Scripts/repo-pilot.exe' if os.name == 'nt' else 'bin/repo-pilot')))
        if args.install_mode == 'editable':
            print('Keep this source checkout in place. Source/payload changes apply on the next launcher run; dependency changes need setup again.')
        if args.mode == 'conda':
